...
The main tradeoff is that we keep a small buffer helper instead of hiding buffers behind a UI abstraction. That makes buffer ownership and option scope explicit, avoids accidental global option writes, and keeps diff previews in plugin-created windows so real file windows do not inherit preview-only options such as `scrollbind` or `cursorbind`.
